...
describe('', () => {
  it('cypress should fail on console.error', async () => {
    const spec = ' --spec ./cypress/integration/shouldFailOnConsoleError.js';
    let testResult = '';
    ...
    try {
      ...
      const expectedError = 'expected error to have been called exactly "0 times", but it was called once';
  ...
  it('cypress should pass without failOnConsoleError config', async () => {
    const spec = ' --spec ./cypress/integration/shouldPassWithoutFailOnConsoleErrorConfig.js';
